...
# Install the Human package
if (!("Mus.musculus" %in% installed.packages())) {
  # Install this package if it isn't installed yet
  BiocManager::install("Mus.musculus", update = FALSE)
}
...
expression_df <- readr::read_tsv("GSE183516_Normalized_counts_H23_H358_A549_cell_lines.txt")
...
mapped_list <- mapIds(
  Mus.musculus, # Replace with annotation package for your organism
  keys = expression_df$Gene,
  keytype = "ENSEMBL", # Replace with the type of gene identifiers in your data
  column = "SYMBOL", # The type of gene identifiers you would like to map to
  multiVals = "list"
